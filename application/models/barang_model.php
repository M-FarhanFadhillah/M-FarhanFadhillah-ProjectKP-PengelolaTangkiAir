...
class Barang_Model extends CI_Model
{
	...
	function getAll($filter = null,$limit = 20,$offset = 0, $orderBy, $orderType)
	{
	  	...
	  	if (isset($filter))
	  	{
			if (!empty($filter->keyword))
			{
				  $cond[] = "(lower(".$this->key.") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 ...
								 or lower(b.harga) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(b.quantity) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
		...
		$query = $this->db->query("SELECT SQL_CALC_FOUND_ROWS b.*,k.nama kategori, r.radius
								   FROM ".$this->table." b
	...
	public function get_by($field, $value = "",$obj = false)
	{
		...
		$query = $this->db->query("SELECT b.*,k.nama kategori, r.radius
								   FROM ".$this->table." b
								   LEFT JOIN kategori k on k.id_kategori = b.id_kategori
								   LEFT JOIN radius r ON r.id_radius = b.id_radius
								   $where 
