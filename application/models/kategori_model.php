...
class Kategori_Model extends CI_Model
{
	...
	function getAll($filter = null, $limit = 20, $offset = 0, $orderBy, $orderType)
	{
		...
		$query = $this->db->query("SELECT SQL_CALC_FOUND_ROWS kat.id_kategori, kat.nama
								   FROM " . $this->table . " kat
	...
	public function get_by($field, $value = "", $obj = false)
	{
		...
		$query = $this->db->query("SELECT  kat.id_kategori, kat.nama
								   FROM " . $this->table . " kat
								   $where 
								   ");
	...
	function save($id = "", $data = array(), $insert_id = false)
	{
		...
		if (!empty($id)) {
			$this->db->where($this->key, $data['id_kategori'])->update($this->table, $kategori);
		} else {
